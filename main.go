...
import (
	...
	"github.com/agill17/s3-operator/controllers"
...
func init() {
	...
	_ = agillappsv1alpha1.AddToScheme(scheme)
...
func getSyncPeriod() time.Duration {
	...
	if val, ok := os.LookupEnv(EnvVarSyncPeriod); ok && val != "" {
		syncPeriod, err = strconv.Atoi(val)
		if err != nil {
			setupLog.Error(err, "Failed to convert RESYNC_PERIOD to int, exiting..")
			os.Exit(1)
		}
	}
